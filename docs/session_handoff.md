...
"the valuable output" — **exists but is empty and meaningless**: it was produced
incidentally by a 21-record smoke test of `run.sh --verify`, reports "0
miscategorised", and covers one sentence's variants rather than the corpus. It
should be regarded as not yet produced. See §10.
...
reports/
  ...
  verification_disagreements.md  EMPTY -- artefact of a 21-record smoke test, not a result
...
**Does not meaningfully exist**: `reports/verification_disagreements.md` is
present in the repo but was generated by a 21-record smoke test of
`run.sh --verify` on the single sentence *"Delhi is the capital of India."* It
says "0 miscategorised, 0 clusters". That is not a result — it is an artefact of
a plumbing test. No full-corpus run has happened.

For the record, that smoke test did complete end to end: 21 records, tier 1
rejected 0, the LLM judged all 21, and it returned `verified=true` for 6 and
`false` for 15. Those verdicts come from the same verifier that fails its gate,
